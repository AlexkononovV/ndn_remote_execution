...
//public:
  ...
  Consumer::functionPrefix(std::string pref)
  {
    ...
    consumer_prefix[id]="/consumer/computation/1";
    return id;
  }
  //-----------------------------------------------------------
  ...
  Consumer::execute_explicit(std::string id){
    ...
    if(modes[id]){
      ...
      auto reg = std::chrono::duration_cast<std::chrono::microseconds>(after - before).count();
      std::cout << "REG PREFIX ms: " << reg << "\n";
      ...
      TM_start1 = std::chrono::steady_clock::now();
      std::ofstream outfile;
      auto boot = std::chrono::duration_cast<std::chrono::microseconds>(TM_start1 - TM_beforexecute).count();
                outfile.open("tests/TOTAL_C_N_boot.txt", std::ios_base::app); // append instead of overwrite
                outfile << boot;
                outfile << "\n";
  ...
  Consumer::run_prefix(std::string id)
  {
    ...
    m_face.processEvents(ndn::time::milliseconds(1),false);
  }
  //-----------------------------------------------------------
          ...
          if( prod_id != ""){
            if(res != ""){
              if (producer_id.find(prod_id) == producer_id.end()){
                ...
                auto completion_time = std::chrono::duration_cast<std::chrono::microseconds>(TM_afterexecute - TM_beforexecute).count();
                auto fetch_time = std::chrono::duration_cast<std::chrono::microseconds>(TM_afterexecute - TM_beforfetch).count();
                //std::ofstream outfile;
                //std::cout << "|||||||||||||||||||||||||||||||||||||||||||||||||||";
                outfile.open("tests/TOTAL_C_N.txt", std::ios_base::app); // append instead of overwrite
                
                outfile << fetch_time;
                outfile << ",";
                outfile << completion_time;
                outfile << "\n";


                outfile.open("tests/count_packets.txt", std::ios_base::app);
    ...
    if( interest.hasApplicationParameters() ) {
      ...
      switch(type){
        case 2201:{
          ...
          TM_end1 = std::chrono::steady_clock::now();
          auto exec_time = std::chrono::duration_cast<std::chrono::microseconds>(TM_end1 - TM_start1).count();
          std::ofstream outfile;
          
          outfile.open("tests/TOTAL_C_N_exec.txt", std::ios_base::app); // append instead of overwrite
          outfile << exec_time;
          outfile << "\n";
  ...
  Consumer::fetchResult(std::string id405){
    TM_beforfetch = std::chrono::steady_clock::now();
