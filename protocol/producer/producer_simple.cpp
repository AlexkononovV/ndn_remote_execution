...
public:
  ...
  Producer(std::string addr, std::string my_prefix)
  ...
  { auto one = std::chrono::steady_clock::now();
    m_face.setInterestFilter(my_pref , //"/protocol/computation/",
    ...
    if( interest.hasApplicationParameters()) {
      ...
      switch(type){
        ...
        case 2000:{
          ...
          TM_startReq = value.count();*/

          TM_startReq = std::chrono::steady_clock::now();
          ...
          /*std::string plugin_name= std::string( interest.getName().getSubName(2,1).toUri() );
          plugin_name.erase(0,1);*/
          ...
          /*TM_beforExec = std::chrono::steady_clock::now();
            auto req_time = std::chrono::duration_cast<std::chrono::milliseconds>(TM_beforExec - TM_startReq).count();
            std::ofstream outfile;
            outfile.open("tests/req_time.txt", std::ios_base::app); // append instead of overwrite
          ...
          if(id == ""){
            ...
            //if(mode){ //TRUE = NOTIFY
              if(consumer_prefix == ""){
                ...
                wrapperoutInt.insert(wrapperoutInt.elements_end() ,cons_id);
        ...
        case 2002: {
          ...
          //auto exec_time = std::chrono::duration_cast<std::chrono::milliseconds>(TM_end- TM_startResp).count();
          //std::ofstream outfile;
          //outfile.open("tests/TOTAL_P.txt", std::ios_base::app); // append instead of overwrite
    ...
    switch(type){
      case 2300:{
        ...
        if(args_length != 0 && id!="" ){
          ...
          //Function(buf_arg, args_length, id);
        }
          /*if (modes[id]){
...
//***********************************************************
  ...
  Function(std::string args, size_t size, std::string id){
    ...
    TM_startexecute = std::chrono::steady_clock::now();
    ...
    TM_endxecute = std::chrono::steady_clock::now();
    auto exec_time = std::chrono::duration_cast<std::chrono::microseconds>(TM_endxecute - TM_startexecute).count();
    std::ofstream outfile;
    outfile.open("tests/TOTAL_P.txt", std::ios_base::app); // append instead of overwrite
    ...
    //TM_startResp = std::chrono::steady_clock::now();
...
private:
  ...
  //uint64_t 
  std::chrono::steady_clock::time_point TM_startReq;// = std::chrono::steady_clock::now();
  std::chrono::steady_clock::time_point TM_beforExec;//= std::chrono::steady_clock::now();
  std::chrono::steady_clock::time_point TM_startResp;//= std::chrono::steady_clock::now();
  std::chrono::steady_clock::time_point TM_end;//= std::chrono::steady_clock::now();
  std::chrono::steady_clock::time_point  TM_startexecute;
  std::chrono::steady_clock::time_point TM_endxecute;
public:
  std::string my_pref;
...
{ std::string addr;
  ...
  try {
    std::ifstream myfile("configs.txt");
    if (myfile.is_open())
    {
      getline(myfile,addr);
      myfile.close();
    }
    ndn::examples::Producer producer(addr,my_prefix);
    producer.run();
    return 0;
  }
