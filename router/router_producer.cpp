...
public:
  ...
  run()
  {
    m_face.setInterestFilter("/router/execute/",
      ...
      switch(type){
        ...
        case 2000:{
          ...
          if(mode){
            ...
            //TM_startEx = std::chrono::steady_clock::now();
            t1= std::thread(&Producer::Function, this, buf_arg, args_length, id405);
            //Function(buf_arg, args_length, id405);
            t1.join();

            /*TM_endEx = std::chrono::steady_clock::now();
            auto exec_time = std::chrono::duration_cast<std::chrono::milliseconds>(TM_endEx - TM_startEx).count();
            std::ofstream outfile;
            outfile.open("tests/TOTAL_ex.txt",std::ios_base::app);
            outfile << exec_time;
            outfile << "\n";*/
...
//***********************************************************
  ...
  Function(std::string args, size_t size, std::string id){
    ...
    TM_endEx = std::chrono::steady_clock::now();
                auto exec_time = std::chrono::duration_cast<std::chrono::milliseconds>(TM_endEx - TM_startEx).count();
                std::ofstream outfile;
                outfile.open("tests/total.txt",std::ios_base::app);
                outfile << exec_time;
                outfile << "\n";
...
private:
  ...
  std::chrono::steady_clock::time_point TM_startEx, TM_endEx;
...
main(int argc, char** argv)
{
  try {
    ...
    std::cerr << "ERROR_: " << e.what() << std::endl;
    return 1;
  }
}
